...
public class PatchAnalyser {
    ...
    //Alternate public constructor for chaining
     public PatchAnalyser(File source, String patchText, File packageDir, String className, String classPath, String testClassName) {
        ...
        Logger.info("patch analyser initialised with testClassName: " + testClassName);
    }
    ...
    private void analyse() {
        ...
        // Dump node numbering to a file
        String nodeNumbering = sourceFileTree.nodeList();
        String nodeFilename = source + ".nodes";
        try {
            FileUtils.writeStringToFile(new File(nodeFilename), nodeNumbering, Charset.defaultCharset());
        } catch (IOException e) {
            Logger.error("Could not write nodes to " + nodeFilename);
            Logger.trace(e);
            System.exit(-1);
        }

        Logger.info("Node numbering written to: " + nodeFilename);
        ...
        //long originalExecutionTime = testRunner.runTests(emptyPatch, WARMUP_REPS).totalExecutionTime();
        ...
        String originalSource = emptyPatch.apply();

        // Write the original source to file, for reference
        String originalFilename = source + ".original";
        try {
            FileUtils.writeStringToFile(new File(originalFilename), originalSource, Charset.defaultCharset());
        } catch (IOException e) {
            Logger.error("Could not write original source to " + originalFilename);
            Logger.trace(e);
            System.exit(-1);
        }
